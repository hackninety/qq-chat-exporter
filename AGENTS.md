...
The production plugin is Rust-only after its thin ESM bridge: packaged runtime
startup is `index.mjs` → `runtime/ApiLauncher.mjs` →
`runtime/rustBridge.mjs` → `qce-server`. Do not add export, parser, resource,
persistence, or API behavior back under the plugin package.
...
2. Confirm which implementation is authoritative; exporter and API behavior
   belong in the Rust crates, not in a parallel plugin implementation.
...
The test harness is login-free and uses `MockNapCatCore`; prefer it over a real
QQ account. `npm run test:runtime` additionally requires a release build of
`qce-server`. See `plugins/qq-chat-exporter/__tests__/README.md`.
...
Also inspect changed lockfiles and generated assets. Do not commit `.next/`,
`dist/`, `target/`, temporary exports, or test credentials.
